...
const coins = [
  ...
  const ChainDropdown = () => {
    ...
    const [selected, setSelected] = useState(coins[4]); // Default to Ethereum
  
    const toggleDropdown = () => {
      setIsOpen((prev) => !prev);
    };
  
    const selectChain = (coin: { name: string; image: any; token: string }) => {
      setSelected(coin);
      setIsOpen(false);
    };
      ...
      <div className="relative  ">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className='text-sm font-bold'>{selected.name} </span> <span className='text-tiny'>{selected.token}</span>
          </div>
          <Image src={Dropdown} alt="Dropdown" width={20} height={20} className={`${isOpen ? 'rotate-180' : ''} transition-transform`} />
        </button>
